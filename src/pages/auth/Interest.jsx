...
const Interest = () => {
  ...
  if (selectedInterest) {
    ...
    const isAlreadySelected = selectedInterests.some((interest) => interest.id === id);
...
const { isLoading } = useMutation(
  ...
  const isInterestSelected = (id) => {
    return selectedInterests.includes(id);
  };
  ...
  {interest.map(({ id, name }) => (
    <div
      className={`interest__categories ${isInterestSelected(id) ? 'selected' : ''}`}
      key={id}
      onClick={() => handleInterestClick(id)}
    >
      <div className={`interest__content ${isInterestSelected(id) ? 'isselected' : ''}`}>
        <p>{name}</p>
        {isInterestSelected(id) ? <AiOutlineCheck /> : <AiOutlinePlus />}
      </div>
    </div>
  ))}
</div>
            <button className={buttonClass} disabled={isButtonDisabled} onClick={handleSubmit}>{isLoading ? 'Submitting...' : 'Go Home'}</button>
      </div>
