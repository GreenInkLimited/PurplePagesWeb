...
export const RevokeAccess = async ({ business_id, username }) => {
  ...
  const response = await axios.post(
    `${API.host}/business/revoke-access/`,
    {
      business_id,
...
export const AddAdmin = async ({ username, business_id }) => {
  ...
  try {
    const response = await axios.post(`${API.host}/business/share-access/`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
export const getSubcriptions = async ({ pageParam = 0 }) => {
  ...
  const response = await axios.get(`${API.host}/business/subscriptions/${auth_code}/${start}/${end}/`);
  return response.data;
};
...
export const getMyBusinessById = async ({id, pageParam = 0}) => {
  ...
  let end = start + step;
  const auth_code = localStorage.getItem('auth_code');
  const response = await axios.get(`${API.host}/business/get-my-business/${auth_code}/${id}/${start}/${end}/`);
  return response.data;
};


export const AddNewProduct = async ({ image, caption, price, discount, color, detail, business_id }) => {
...
export const WriteBlog = async ({ title, detail, image, tags, business_id }) => {
...
export const PromoteBusiness = async ({ ads_type, price, plan, duration, location, business_id }) => {
  ...
  try {
    const response = await axios.post(`${API.host}/business/promote/`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
export const SendMail = async ({ subject, mail, file, price, bundle, business_id }) => {
