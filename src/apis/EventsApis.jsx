...
export const BuyTicket = async ({ quantity, first_name, last_name, email, id}) => {
...
export const CreateEventAccount = async ({full_name, username, email, bank_name, account_type, account_name, account_number, password}) => {
...
export const AddEventDetails = async ({ event_time, start_time, end_time, frequency, days, end_date, event_name, event_description, event_category, event_location, event_frequency, from_date, to_date, choose_dates, ticket_type, ticket_name, quantity, ticket_price, event_flier }) => {
  ...
  try {
    const response = await axios.post(`${API.host}/event/post-event/`, formData, {
      headers: {
...
export const SearchEvents = async (searchQuery) => {
  ...
  const response = await axios.get(`${API.host}/event/search-event/?q=${searchQuery}`);
  return response.data;
};