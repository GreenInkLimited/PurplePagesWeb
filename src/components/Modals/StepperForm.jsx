...
const StepperForm = () => {
  ...
  const { isLoading, error, isError, mutateAsync, data } = useMutation('create event', AddEventDetails, {
  onSuccess: (data) => {
    if (data && data.success) { // Adjust the condition based on the response data structure
      navigate('/events');
    } else {
  ...
  return (
    <div>
      ...
      {currentStep === 2 && (
        <CreateTicket onNext={handleNext} onPrev={handlePrev} formData={formData}/>
      )}
      {currentStep === 3 && (
        <PublishEvent onPrev={handlePrev} onSubmit={handleSubmit} formData={formData}/>
      )}
    </div>
  );
};
