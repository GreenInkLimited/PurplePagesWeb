...
const handleCheckboxChange = (value) => {
    if (adsContent.includes(value)) {
      setAdsContent((prevContent) =>
        prevContent.filter((content) => content !== value)
      );
    } else {
      setAdsContent((prevContent) => [...prevContent, value]);
    }
  };
...
const handleLogoUpload = (event) => {
  ...
  async (values) => {
    try {
      ...
      if (adsResponse.status_lean) {
        const adsId = adsResponse.ads_id; // Get the ID of the created ads

        const categoryResponse = await AddCategory({
          ads_id: adsId,
          category: adsContent, // Assuming adsContent is an array of selected categories
        });

        console.log('AddCategory response:', categoryResponse);

        if (categoryResponse.status_lean) {
          // AddCategory mutation was successful
          navigate('/personal');
        } else {
          // AddCategory mutation failed
          throw new Error(categoryResponse.detail);
        }
      } else {
        // AddAds mutation failed
        throw new Error(adsResponse.detail);
      }
    } catch (error) {
      console.error('Error:', error);
      throw error; // Re-throw the error to be handled by the error boundary or catch block
    }
  },
  {
    onError: (error) => {
 ...
 // console.error('Payment failed:', error);
                    ...
                    {locations.map((option) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          field.onChange({ target: { value: option, name: 'location' } });
                        }}
  ...
  <ErrorMessage name="location" component="small" className="error-message" />
  <div>
    <label>Choose Duration</label>
    <Field className="input" type="text" name="duration" placeholder="If applicable" />
     <ErrorMessage name="duration" component="small" className="error-message" />
                </div>
            </div>
            ...
            <div className='post__ads-wrapper'>
              <label>
                <Field
                    ...
                    name="adsContent"
                    value="Brand Ad"
                    checked={adsContent.includes('Brand Ad')}
            ...
            <div className='post__ads-wrapper'>
              <label>
                <Field
                    ...
                    name="adsContent"
                    value="Product Ad"
                    checked={adsContent.includes('Product Ad')}
            ...
            <div className='post__ads-wrapper'>
              <label>
                <Field
                    ...
                    name="adsContent"
                    value="Sales"
                    checked={adsContent.includes('Sales')}
            ...
            <div className='post__ads-wrapper'>
              <label>
                <Field
                    ...
                    name="adsContent"
                    value="Promo"
                    checked={adsContent.includes('Promo')}
            ...
            <div className='post__ads-wrapper'> 
              <label>
                <Field
                    ...
                    name="adsContent"
                    value="Discount"
                    checked={adsContent.includes('Discount')}
            ...
            <div className='post__ads-wrapper'>
              <label>
                <Field
                    ...
                    name="adsContent"
                    value="Black Friday"
                    checked={adsContent.includes('Black Friday')}
            ...
            <div className='post__ads-wrapper'>
              <label>
                <Field
                    ...
                    name="adsContent"
                    value="Others"
                    checked={adsContent.includes('Others')}
         ...
         <label>Upload your logo (optional)</label>
          ...
          <Field className="input" type="text" name="title" placeholder="Enter title" />
           <ErrorMessage name="title" component="small" className="error-message" />
          
          <label>Advert Caption</label>
          <Field as="textarea" className="input" type="text" name="detail" placeholder="Your advert caption" />
           <ErrorMessage name="detail" component="small" className="error-message" />
